...
| `LW-BLK-009` | P1 | P4-P10 | Exact primary-Chat onboarding/provider/send/stream/cancel/retry/persistence/recovery/privacy/accessibility baseline behavior is not characterized | Execute only `LW-P4-CHAR-001` additive synthetic-profile, denied-egress harness after preflight lock | Codex root controller; independent QA reviewer | All 39 mandatory atomic subcases in 16 groups and all 14 gates PASS with accepted manifest and independent clean-worktree reproduction | Remove additive harness and unaccepted staging; retain accepted evidence with invalidation receipt | OPEN |
