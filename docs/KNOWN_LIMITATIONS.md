...
| `LIM-004` | Data | Storage names are inventoried but schemas/owners/migrations are incomplete; ADR-004 is accepted only for the synthetic conversation slice | Candidate must not mutate legacy data | Run legacy only for stored-data workflows | Expand descriptors and verify synthetic migration/rollback fixtures before any later real-data decision | Open |
...
| `LIM-007` | Security/privacy | Provider/proxy contracts in ADR-005/006 are accepted, but gateway, credentials, workers, peer, GitHub, Drive, Telegram, and wallet boundaries remain source-mapped only | Trust and failure behavior are unknown | Do not enable or mutate external services | Verify deterministic mock/provider contracts; ADR-012 and later proxy/LAN security tests remain required | Open |
