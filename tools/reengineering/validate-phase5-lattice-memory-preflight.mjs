...
const PACKET_PATH = "reengineering/PHASE5_LATTICE_MEMORY_PREFLIGHT.md";
const SCHEMA = "latticework.phase5-lattice-memory-preflight.v1";
...
const OBSERVATION_GROUPS = Object.freeze([
...
const OWNED_EXACT = Object.freeze([
...
function assertExact(lock, root) {
  assert.equal(lock.accepted, true);
  assert.equal(lock.work_id, "LW-P5-MEM-PREFLIGHT-001");
  assert.equal(lock.characterization_work_id, "LW-P5-MEM-CHAR-001");
  assert.equal(lock.implementation_work_id, "LW-P5-MEM-001");
  assert.equal(lock.base_commit, BASE_COMMIT);
  assert.equal(lock.phase4_terminal_commit, "1b7e1d10456e0a1e9aaa91df25db17e236bbea3e");
  assert.equal(lock.baseline_sha, BASELINE_SHA);
  assert.deepEqual(lock.legacy_module, {
  ...
  assert.equal(
    ...
    MODULE_SHA256,
    "legacy module hash drifted",
  );
  assert.equal(lock.feature_id, "lattice-memory-pulse-medium");
  assert.deepEqual(lock.storage, {
    ...
    max_records: 10000,
    max_pending: 100,
  }, "storage contract drifted");
  assert.deepEqual(lock.pulse_contract.allowed_keys, [
    "ts",
    "source",
    "kind",
    "summary",
  ...
  assert.deepEqual(
    lock.pulse_contract.reserved_sources,
    ["quiet-room"],
    "reserved_sources drifted",
  );
  assert.equal(
    lock.pulse_contract.max_summary,
    80,
    "max_summary drifted",
  );
  assert.equal(lock.pulse_contract.max_refs, 16);
  assert.deepEqual(lock.pulse_contract.internal_keys_never_exposed, ["_id"]);
  assert.deepEqual(
    lock.public_api,
    PUBLIC_API,
    "public_api drifted",
  ...
  assert.deepEqual(
    lock.required_observation_groups,
    OBSERVATION_GROUPS,
    "required_observation_groups drifted",
  );
  assert.equal(lock.required_atomic_observations.length, 69);
  assert.equal(new Set(lock.required_atomic_observations).size, 69);
  assert.deepEqual(
    lock.required_atomic_observations,
    Object.keys(ATOMIC_CONTRACTS),
    "required_atomic_observations drifted",
  );
  assert.deepEqual(
    lock.atomic_contracts,
    ATOMIC_CONTRACTS,
    "atomic_contracts drifted",
  );
  assert.deepEqual(lock.browser_contract, {
    channel: "chrome",
    workers: 1,
    retries: 0,
    ...
    beacon_denied: true,
  }, "browser_contract drifted");
  assert.deepEqual(lock.fixture_listener, {
  ...
  }, "fixture_listener drifted");
  assert.deepEqual(lock.required_gates, REQUIRED_GATES, "required_gates drifted");
  assert.deepEqual(
    lock.characterization_owned_exact_paths,
    OWNED_EXACT,
    "characterization_owned_exact_paths drifted",
  );
  assert.deepEqual(lock.characterization_owned_prefixes, [
    "reengineering/evidence/phase-5/LW-P5-MEM-CHAR-001/",
  ]);
  assert.deepEqual(
    lock.protected_exact_paths,
    PROTECTED_EXACT,
    "protected_exact_paths drifted",
  );
  assert.deepEqual(
    lock.protected_prefixes,
    PROTECTED_PREFIXES,
    "protected_prefixes drifted",
  );
  for (const flag of [
    ...
    "candidate_storage_authorized",
    "activation_authorized",
    "deployment_authorized",
    "cutover_authorized",
  ]) {
    assert.equal(lock[flag], false, `${flag} must remain false`);
  }
}

export function validatePhase5LatticeMemoryPreflight({ workspaceRoot }) {
  const root = path.resolve(workspaceRoot);
  const failures = [];
  let lock = null;
  try {
    lock = extractLock(fs.readFileSync(path.join(root, PACKET_PATH), "utf8"));
    assertExact(lock, root);
  } catch (error) {
    failures.push(error.message);
  }
  return {
    schema: "latticework.phase5-lattice-memory-preflight-validation.v1",
    valid: failures.length === 0,
    packet: PACKET_PATH,
    baseCommit: lock?.base_commit ?? null,
    baselineSha: lock?.baseline_sha ?? null,
    atomicObservations: lock?.required_atomic_observations?.length ?? 0,
    observationGroups: lock?.required_observation_groups?.length ?? 0,
    implementationAuthorized: lock?.implementation_authorized ?? null,
    failures,
  };
}
...
if (
  ...
  if (process.argv.length !== 2) {
    process.stderr.write(
      "Phase 5 LatticeMemory preflight validator accepts no CLI overrides.\n",
    );
    process.exitCode = 2;
  } else {
    const result = validatePhase5LatticeMemoryPreflight({
